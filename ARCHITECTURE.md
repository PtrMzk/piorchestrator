...
- **No isolation layer**: agents run directly on the host with `--permission-mode bypassPermissions`, confined only by the git worktree they are given as a working directory. Isolation is deliberately out of scope for this project — run `po` inside whatever sandbox or VM you trust if you need it
