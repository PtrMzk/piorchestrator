...
| **Sandbox** | `sandbox/provider.py`, `sandbox/docker.py` | Docker isolation (default) with domain-level network firewall |
...
- **Integration testing with mock Claude binary**: `tests/mock_claude.py` impersonates the `claude` CLI, accepting the same flags and emitting stream-json JSONL. It detects spec-generation prompts (returns a canned 3-task spec), task-execution prompts (writes files + git commits in the worktree), and falls back to a generic success. `tests/test_integration.py` injects this mock onto `$PATH` and exercises the full `po init` → `po run` flow through real subprocess invocations, covering `_invoke_claude()` (sync) and `ClaudeCodeRunner.run()` (async) code paths that E2E tests skip. A Docker variant (`test_full_init_to_run_flow_docker`, marked `@pytest.mark.docker`) builds a lightweight test image with mock claude baked in and runs the full flow through real Docker containers, exercising `DockerSandbox.wrap_command()`, `entrypoint.sh` iptables/su-exec setup, and the container lifecycle. Deselect with `-m 'not docker'`
- **Docker sandbox (default)**: Agents run inside Docker containers by default. Project root is mounted at the same absolute path (preserving worktree references). Network is firewalled via iptables to allow HTTPS only to `api.anthropic.com` and package registries (`pypi.org`, `files.pythonhosted.org`, `registry.npmjs.org`); DNS (port 53) is allowed so package managers can resolve CDN hostnames; all other outbound traffic is dropped. The firewall is hardened: default iptables policy is set to DROP (not trailing REJECT — survives rule flushes), IPv6 is dropped via both sysctl and `ip6tables -P DROP`, DNS-resolved IPs are validated against a regex before use, the entrypoint fails hard if no allowed IPs are found (prevents running without isolation), and firewall correctness is verified at startup by confirming a non-allowlisted host (`example.com`) is blocked and an allowlisted host (`api.anthropic.com`) is reachable. Auth credentials are stored in a named Docker volume (`po-claude-auth`); on first use, the user is prompted to log in interactively inside a one-off container, and the volume persists across all subsequent runs. The container includes Python 3 + uv and ripgrep for agent use. Disable with `--no-sandbox`. The `SandboxProvider` protocol allows swapping isolation backends without changing the orchestration logic
