"""Tests for sandbox provider protocol, Docker wrapping, and seatbelt sandboxing."""
...
class TestCheckSandboxExec:
    def test_not_available(self) -> None:
        with (
            patch("po.sandbox.seatbelt.shutil.which", return_value=None),
            pytest.raises(SandboxError, match="not available"),
        ):
            _check_sandbox_exec()

    def test_available(self) -> None:
        with patch("po.sandbox.seatbelt.shutil.which", return_value="/usr/bin/sandbox-exec"):
            _check_sandbox_exec()  # Should not raise


class TestBuildProfile:
    def test_contains_deny_default(self, tmp_path: Path) -> None:
    ...
    def test_allows_file_reads(self, tmp_path: Path) -> None:
        profile = _build_profile(
            project_root=tmp_path / "project",
            worktree_path=tmp_path / "worktree",
            home=tmp_path / "home",
    ...
    def test_restricts_file_writes_to_project(self, tmp_path: Path) -> None:
        project = tmp_path / "project"
        project.mkdir()
        profile = _build_profile(
            project_root=project,
            worktree_path=project / ".po" / "worktrees" / "task-1",
            home=tmp_path / "home",
        )
        # Project root should be in the write-allowed paths
        assert f'(subpath "{project.resolve()}")' in profile

    def test_allows_temp_dir_writes(self, tmp_path: Path) -> None:
        profile = _build_profile(
            project_root=tmp_path,
            worktree_path=tmp_path,
            home=tmp_path / "home",
    ...
    def test_allows_home_caches(self, tmp_path: Path) -> None:
        home = tmp_path / "home"
        home.mkdir()
        profile = _build_profile(
            project_root=tmp_path,
        ...
        assert f'(subpath "{home.resolve()}/.claude")' in profile
        assert f'(subpath "{home.resolve()}/.npm")' in profile
        assert f'(subpath "{home.resolve()}/.cache")' in profile

    def test_allows_https_only(self, tmp_path: Path) -> None:
        profile = _build_profile(
            project_root=tmp_path,
            worktree_path=tmp_path,
            home=tmp_path / "home",
        ...
        assert '(allow network-outbound (remote tcp "*:53"))' in profile
        # Should NOT allow arbitrary ports like HTTP
        assert '"*:80"' not in profile
        assert '"*:22"' not in profile
...
class TestSeatbeltSandbox:
    def test_wrap_command_structure(self, tmp_path: Path) -> None:
        ...
        project = tmp_path / "project"
        project.mkdir()
        worktree = project / ".po" / "worktrees" / "task-1"
        ...
        # Command starts with sandbox-exec
        assert new_cmd[0] == "sandbox-exec"
        assert new_cmd[1] == "-f"
        # Profile file path is the third argument
        profile_path = new_cmd[2]
    ...
    def test_wrap_command_writes_profile_file(self, tmp_path: Path) -> None:
        sandbox = SeatbeltSandbox()
        project = tmp_path / "project"
        project.mkdir()
        ...
        new_cmd, _ = sandbox.wrap_command(
            cmd, worktree_path=project, project_root=project, env=env,
        )

        # Profile file should exist and contain valid profile
        profile_path = new_cmd[2]
    ...
    def test_wrap_command_env_passthrough(self, tmp_path: Path) -> None:
        """Env should be passed through (agent inherits host auth)."""
        sandbox = SeatbeltSandbox()
        env = {"HOME": "/Users/test", "ANTHROPIC_API_KEY": "sk-test", "FOO": "bar"}

        _, new_env = sandbox.wrap_command(
            ["claude"], worktree_path=tmp_path, project_root=tmp_path, env=env,
        )

        # All env vars should pass through (no Docker-style env stripping)
        assert new_env == env

    def test_wrap_command_uses_env_home(self, tmp_path: Path) -> None:
        """Profile should use HOME from env, not system home."""
        sandbox = SeatbeltSandbox()
        ...
        with open(new_cmd[2]) as f:
            profile = f.read()
        assert str(custom_home.resolve()) in profile

    @pytest.mark.asyncio
    async def test_prepare_checks_sandbox_exec(self) -> None:
        sandbox = SeatbeltSandbox()
        with patch("po.sandbox.seatbelt._check_sandbox_exec") as mock_check:
            await sandbox.prepare()
        mock_check.assert_called_once()

    @pytest.mark.asyncio
    async def test_prepare_raises_when_unavailable(self) -> None:
        sandbox = SeatbeltSandbox()
        with (
            patch("po.sandbox.seatbelt.shutil.which", return_value=None),
            pytest.raises(SandboxError, match="not available"),
    ...
    def test_profile_reuses_temp_dir(self, tmp_path: Path) -> None:
        """Multiple wrap_command calls should reuse the same temp dir."""
        sandbox = SeatbeltSandbox()
        env = {"HOME": str(tmp_path)}

        cmd1, _ = sandbox.wrap_command(
            ["claude"], worktree_path=tmp_path, project_root=tmp_path, env=env,
        )
        cmd2, _ = sandbox.wrap_command(
            ["claude"], worktree_path=tmp_path, project_root=tmp_path, env=env,
        )

        # Both should use files in the same temp directory
        assert Path(cmd1[2]).parent == Path(cmd2[2]).parent
